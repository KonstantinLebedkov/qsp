...
{ // есть нормальный компаратор, реализованный как метод стринга. не вижу смысла писать десяток идентичных процедур
...
{//см.выше, но сравниваются обрезки строк размерностью длины второй строки.
...
{
    QSPVariant compValues[QSP_MAXITEMS]; //объявляет массив вариантов, размерностью 200. нахуа?
    QSP_TINYINT compOpCodes[QSP_MAXITEMS], compArgsCounts[QSP_MAXITEMS]; // объявляет 2 массива чаров, размерностью 200 каждый.
    int itemsCount;
